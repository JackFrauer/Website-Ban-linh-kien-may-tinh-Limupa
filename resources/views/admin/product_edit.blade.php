    ...
    <div id="content">
        <div class="container-fluid">
            <form method="post" action="/admin/edit/{{$product->id}}" enctype="multipart/form-data">
                ...
                <textarea name="product_details" id="product_details_editor">{{ $product->product_details }}</textarea>
                ...
                <input type="text" name="product_year" class="form-control" value="{{ $product->product_year }}">
                @error('product_year')
                    <p style="color: red; font-size: 14px; margin-top: 5px;">{{ $message }}</p>
                @enderror
                <br>
                ...
                <textarea name="product_description" id="product_description_editor">{{ $product->product_description }}</textarea>
                @error('product_description')
                    <p style="color: red; font-size: 14px; margin-top: 5px;">{{ $message }}</p>
                @enderror

                <label>Chi tiết thêm:</label>
                <textarea name="more_details" id="more_details_editor">{{ $product->more_details }}</textarea>
                @error('more_details')
                    <p style="color: red; font-size: 14px; margin-top: 5px;">{{ $message }}</p>
                @enderror
